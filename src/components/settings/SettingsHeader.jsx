...
const SettingsHeader = ({ searchQuery, onSearchChange, profileSummary }) => (
  <header className="rounded-lg border border-[#ebebeb] bg-white p-4 shadow-[0_1px_1px_rgba(0,0,0,0.03),0_8px_16px_-12px_rgba(0,0,0,0.08)] sm:p-6">
    <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
      ...
      <div className="flex w-full flex-col gap-3 xl:w-[420px]">
        ...
        <div className="flex min-h-16 items-center gap-3 rounded-lg border border-[#ebebeb] bg-[#fafafa] p-3">
          {profileSummary.avatarUrl ? (
            <img src={profileSummary.avatarUrl} alt={`${profileSummary.name} avatar`} className="h-11 w-11 rounded-md object-cover" />
          ) : (
            <div className="flex h-11 w-11 items-center justify-center rounded-md bg-[#171717] text-sm font-semibold text-white">
          ...
          <span className="rounded-md border border-[#d3e5ff] bg-white px-2 py-1 text-xs font-medium text-[#0761d1]">
            {profileSummary.statusLabel}
          </span>
        </div>
      </div>
    </div>
